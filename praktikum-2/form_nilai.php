...
if(isset($_POST['submit']) > 0) {
  $hasil = (0.25*(int)$_POST['uts'])+(0.5*(int)$_POST['uas'])+(0.25*(int)$_POST['tugas']);
}
?>
<!DOCTYPE html>
<html lang="en">
      ...
      <div class="col-md-5 mx-auto">
          ...
          <form action="" method="POST">
              <div class="wrapper">
                  ...
                  <div class="stats">
                      <div>
                          <strong>NAMA</strong><?= $_POST['nama'] ?? 'Kosong' ?>
                      </div>

                      <div>
                          <strong>NIM</strong><?= $_POST['nim'] ?? 'Kosong' ?>
                      </div>

                      <div>
                          <strong>Matkul</strong><?= $_POST['matkul'] ?? 'Kosong' ?>
                      </div>
                  ...
                  <div class="stats">
                      ...
                      <div>
                          <strong>SKS</strong><?= $_POST['sks'] ?? 'Kosong' ?>
                      </div>

                      <div>
                          <strong>Nilai UTS</strong><?= $_POST['uts'] ?? 'Kosong' ?>
                      </div>

                      <div>
                          <strong>Nilai UAS</strong><?= $_POST['uas'] ?? 'Kosong' ?>
                      </div>
                  ...
                  <div class="stats">
                      ...
                      <div>
                        <strong>Nilai Tugas</strong><?= $_POST['tugas'] ?? 'Kosong' ?>
                      </div>
                      <div>
                        <strong>Bobot</strong><?= $hasil ?? 'Kosong' ?>
                      </div>
                      <div>
                        IP: <?= getIp($hasil) ?? 'Kosong' ?></br>
                        Grade: <?= getGrade($hasil) ?? 'Kosong' ?>
                      </div>
                  </div>

                  <div class="footer">
                    <button type="reset" class="Cbtn Cbtn-danger">Reset</button>
                    <input type="submit" name="submit" class="Cbtn Cbtn-primary">
                  </div>
              </div>
          </div> 
          </form>
